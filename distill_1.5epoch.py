...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        # 新增：在1.5个epoch时额外保存
        current_step = epoch * iter_per_epoch + step  # 计算当前总步数（关键！） 注意epoch应该是0，1序列
        if current_step == total_steps:  # 1.5 epoch对应的步数:
            model.eval()
            ckp = f'{args.save_dir}/distill_1.5epoch.pth'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_project", type=str, default="KyrinChen-Distill-long")
    ...
    iter_per_epoch = len(train_loader) #iter_per_epoch 表示每个 epoch 的迭代次数，即训练数据被分成多少个 batch（批次）。
    total_steps = int(1.5 * len(train_loader))  # 新增此行：计算总步数并传递
